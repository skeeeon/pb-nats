...
func main() {
	...
	natsOptions.ConnectionRetryConfig = &pbnats.RetryConfig{
	...
	natsOptions.ConnectionTimeouts = &pbnats.TimeoutConfig{
	...
	// Register NATS commands - the export command needs to know the custom
	// account collection name
	cmdOptions := pbnats.DefaultCommandOptions()
	cmdOptions.AccountCollectionName = natsOptions.AccountCollectionName
	pbnats.RegisterCommandsWithOptions(app, cmdOptions)
