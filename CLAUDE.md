...
- **Startup reconciliation**: `sync.Manager.ReconcileAccounts()` enqueues every active account at boot, since account JWTs live only in the NATS resolver directory and hook-driven sync can't recover a lost one. `publisher.ReviveFailedDeletes()` covers the other direction at boot: a queued *delete* that exhausted `MaxQueueAttempts` can never be reconstructed (its account row is gone, so the queue row's public-key snapshot is the only record of the intent), and once `CleanupFailedRecords` sweeps it the account stays live in NATS with nothing left to notice it by
